...
**方式 A：配置文件** `~/.config/smart-search/config.json`
...
**完全自包含**——`smart_search` 模块是 Python 包，`grok_search.py` 直接 `import` 调用，**不依赖外部 smart-search CLI**。
...
```bash
# 检查配置
cat ~/.config/smart-search/config.json
